...
for(i in results){
  ...
  rf_se <- i$rf %>% as.data.frame() %>% replace( is.na(.), 0) %>% map_dbl(function(x) sd(x)/sqrt(length(x)))
...
df_plot <- cbind(sc_feat_umap$layout, cluster) %>% as.data.frame()
...
ggplot(data = df_plot, 
  ...
  guides(size = FALSE) +
  theme_bw() +
  annotate("text", x = 6, y = -7, label = "SC = 1.22", size = 5) +
  annotate("text", x = 0, y = 4, label = "median AUC", size = 5) +
  annotate("text", x = 7, y = 4, label = "median AUC", size = 5) +
  annotate("text", x = -5, y = 0, label = "median AUC", size = 5) +
  annotate("text", x = 5, y = -2, label = "median AUC", size = 5)
...
# Pairwise t-test
pairwise.t.test(sc_feat_all_clust$sc, sc_feat_all_clust$cluster, p.adjust.method ='none')
...
sc_feat_all_clust %>% group_by(cluster) %>% summarize(mean = mean(sc),
                                                      sd = sd(sc), 
                                                      se = sd(sc)/sqrt(n()))
...
sc_feat_all_clust %>% filter(cluster==1) %>% map(mean) %>% unlist()
