...
# compare models
anova(fit_cfa.one, fit_cfa_3f_g)
anova(fit_cfa.ort, fit_cfa_3f_g)
```
...
anova(fit_cfa_2f_orth, fit_cfa_2f_g)
...
summary(df_all_teq$gamesGen)
summary(df_all_teq$gamingSkills)
...
```
## Correlations for different test sequences
```{r}
cor_fist <- test_time_all %>% 
  as.data.frame() %>% 
  filter(first_sess==1) %>% 
  select(pc, mr, sc, VKMR, SPM) %>% 
  cor(method = "spearman")


cor_second <- test_time_all %>% as.data.frame() %>% 
  filter(first_sess==0) %>% 
  select(pc, mr, sc, VKMR, SPM) %>% 
  cor(method = "spearman")

cor_average <- cor_fist * 57/110 + cor_second * 53/110

cor_fist%>% xtable::xtable()
cor_second %>% xtable::xtable()
cor_average  %>% xtable::xtable()
